...
{
	pthread_detach(pthread_self());	//detach very important
...
int main(int argc, char* argv[])
{
	...
	//signal(SIGCHLD,child_handle);	//handle child process
	...
	while(1){
		...
		pthread_create(&tid,NULL,thd_handle,(void*)client_sockfd);

		
		/*
		//child handle for client
		if(fork()==0)
		{
			close(server_sockfd);
